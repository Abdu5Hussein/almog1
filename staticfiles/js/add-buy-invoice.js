document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchLastRecieptNo(callback) {
        ...
        customFetch(`get-last-buyinvoice-id`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
    ...
    document.getElementById("createButton").addEventListener("click", async function (event) {
        ...
        const source_index = document.getElementById("source").selectedIndex;
        const currency_index =
            document.getElementById("currency").selectedIndex;
        if (source_index == 0 || currency_index == 0) {
            alert("الرجاء اختيار المصدر ونوع العملة");
            return;
        }
    ...
    document.getElementById("excell-btn").addEventListener("click", function (event) {
        const invoice = getInputValue("invoice-autoid");
