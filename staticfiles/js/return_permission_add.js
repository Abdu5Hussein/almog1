document.addEventListener("DOMContentLoaded", function () {
    var table = new Tabulator("#return-table", {
        ...
        columns: [
            ...
            { title: "كمية الفاتورة", field: "org_quantity", hozAlign: "Center" },
            { title: "الكمية المرجعة", field: "returned_quantity", hozAlign: "Center" },
            { title: "سبب الترجيع", field: "return_reason", hozAlign: "right" },
            { title: "سعر القطعة", field: "price", hozAlign: "right", formatter: "Center" },
            { title: "الاجمالي", field: "total", hozAlign: "right", formatter: "Center" },
    ...
    async function createPermission() {
        ...
        try {
            ...
            if (response.ok) {
                alert("Permission created successfully! ID: " + result.autoid);
                document.getElementById("createButton").disabled = "true";
                document.getElementById("returnButton").style.display = "block";
                document.getElementById("return-autoid").value = result.autoid;
                addPermissionItems(result.autoid);
            } else {
                alert("Error: " + JSON.stringify(result));
            }
        } catch (error) {
            console.error("Request failed", error);
            alert("Failed to create permission. Check console for details.");
        }
    }
    ...
    document.getElementById('refresh-btn').addEventListener('click', function () {
        const invoice = document.getElementById("invoice").value
        const create_button_disabled = document.getElementById("createButton").disabled

        if (invoice && create_button_disabled) {
            fetchInvoiceItems(invoice);
        }
    });
    window.addEventListener('storage', function (event) {
        if (event.key === 'refresh_return_items' && event.newValue === 'true') {
            ...
            localStorage.setItem('refresh_return_items', 'false');
        }
    });
    document.getElementById("client").addEventListener("change", function () {
        const selectedClient = this.options[this.selectedIndex].dataset.clientName;
        ...
        options.forEach(option => {
            if (!option.dataset.clientName || option.dataset.clientName === selectedClient || option.value === "") {
                option.style.display = "block";  // Show relevant invoices
                console.log("options: ", option);
                console.log("options: ", option.dataset);
            } else {
