document.addEventListener("DOMContentLoaded", function () {
    ...
    const clients_table = new Tabulator("#clients-table", {
        height: "auto", // Adjust height or set a fixed height
        layout: "fitColumns",
        selectable: true,
        rowHeight: 20,
        ...
        columns: [
            // Column definitions
            // Column definitions based on the provided model fields
            {
                title: "الرقم الخاص",
                field: "pno",
                headerMenu: false,
    ...
    table.on("rowClick", function (e, row) {
        ...
        // Fetch data for clients_table
        customFetch(`api/filter-clients/?pno=${clickedPno}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${jwtToken_access}`,  // 👈 Include JWT token
    ...
    function applyFiltersForClients() {
        // Helper function to get selected text from a dropdown
        function getSelectedText(selectId) {
            const select = document.getElementById(selectId);
            return select && select.selectedIndex !== 0
        ...
        const filterValues = {
            itemno: getInputValue("original-no"),
            maintype: getSelectedText("item-main"),
            clientname: getInputValue("client-name"),
            itemname: getInputValue("pname-arabic"),
            pno: getInputValue("pno"),
        ...
        // Perform the fetch request
        customFetch("/api/filter-client-input/", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRFToken": getCSRFToken(),
            ...
            .then((data) => {
                ...
                clients_table.replaceData(data);
            })
            .catch((error) => {
    ...
    ClientsFilterInputs.forEach((inputId) => {
        document
            .getElementById(inputId)
            .addEventListener("input", applyFiltersForClients);
    });
    ...
    function exportToExcel() {
        clients_table.download("xlsx", "clients-table_data.xlsx"); // 'xlsx' is the file format
    }
    ...
    function exportToPDF() {
        ...
        let visibleColumns = clients_table
        ...
        let tableData = clients_table.getData().map((row) => {
        ...
        customFetch("/generate-pdf/", {
            ...
            .then((blob) => {
                ...
                link.href = fileURL;
                link.download = "clients-table_data.pdf";
                link.click();
            })
            .catch((error) => {
